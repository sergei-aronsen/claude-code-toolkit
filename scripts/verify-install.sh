...
# ─────────────────────────────────────────────────
# 7. v6.1 Advisory Hooks (optional)
# ─────────────────────────────────────────────────

section "7. v6.1 Advisory Hooks (optional)"

# Hooks declared by scripts/install-hooks.sh:56-60. Each hook is registered in
# ~/.claude/settings.json with `_tk_hook_id: <basename>` and lives at
# ~/.claude/hooks/<basename>. Verify settings.json marker + file presence.
HOOKS_EXPECTED=(
...
HOOKS_DIR_GLOBAL="$GLOBAL_DIR/hooks"

if [[ -f "$SETTINGS_JSON" ]] && grep -q "_tk_hook_id" "$SETTINGS_JSON" 2>/dev/null; then
    for hk in "${HOOKS_EXPECTED[@]}"; do
        if grep -q "\"_tk_hook_id\": *\"$hk\"" "$SETTINGS_JSON" 2>/dev/null; then
            if [[ -x "$HOOKS_DIR_GLOBAL/$hk" ]]; then
                pass "$hk registered + executable"
            elif [[ -f "$HOOKS_DIR_GLOBAL/$hk" ]]; then
                warn "$hk registered but not executable — run: chmod +x $HOOKS_DIR_GLOBAL/$hk"
            else
                fail "$hk registered in settings.json but missing at $HOOKS_DIR_GLOBAL/$hk"
            fi
        else
            warn "$hk not registered (re-run install-hooks.sh)"
        fi
    done
else
    skip "v6.1 advisory hooks not installed (optional) — install with install-hooks.sh"
fi
...
section "8. v6.1 Cost Routing (optional)"

GLOBAL_CLAUDE_MD="$GLOBAL_DIR/CLAUDE.md"
if [[ -f "$GLOBAL_CLAUDE_MD" ]] && grep -q "BETTER-MODEL ROUTING START" "$GLOBAL_CLAUDE_MD" 2>/dev/null; then
    pass "Routing block present in $GLOBAL_CLAUDE_MD"
    if command -v npx &>/dev/null; then
        pass "npx available (better-model invocation)"
    else
        warn "npx not found — install Node.js to use better-model"
    fi
else
    skip "Cost-routing block not configured (optional) — install with setup-cost-routing.sh"
fi
...
if [[ $FAIL -gt 0 ]]; then
    ...
    echo -e "  ${CYAN}Hooks:${NC}        bash <(curl -sSL https://raw.githubusercontent.com/sergei-aronsen/claude-code-toolkit/main/scripts/install-hooks.sh)"
    echo -e "  ${CYAN}Cost-routing:${NC} bash <(curl -sSL https://raw.githubusercontent.com/sergei-aronsen/claude-code-toolkit/main/scripts/setup-cost-routing.sh)"
fi
