...
| **통합 카탈로그**                     | TUI 인스톨러가 32개 MCP 서버 + 10개 동반 CLI를 10개 카테고리(Backend / Payments / Workspace / Project Management / …)에서 제공. 행마다 scope 선택. |
...
`--integrations` 플래그(또는 첫 설치 후 `/integrations`)가 32개 서버를 10개 카테고리로 보여주는 TUI 체크리스트를 엽니다. 프로젝트에 필요한 것만 고르면 됩니다.
