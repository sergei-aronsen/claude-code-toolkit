...
| **インテグレーションカタログ**        | TUI インストーラで 32 個の MCP サーバー + 10 個のコンパニオン CLI を 10 カテゴリ（Backend / Payments / Workspace / Project Management / …）から選択可能。行ごとに scope。 |
...
`--integrations` フラグ（または初回インストール後の `/integrations`）で、32 個のサーバーを 10 カテゴリ別に並べた TUI チェックリストが開きます。プロジェクトに必要なものだけを取ります。
