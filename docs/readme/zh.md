...
| **集成目录**                      | TUI 安装器，覆盖 32 个 MCP 服务器 + 10 个配套 CLI，分 10 个类别（Backend / Payments / Workspace / Project Management / …）。每行可选 scope。       |
...
`--integrations` flag（或首次安装后用 `/integrations`）打开一个 TUI 检查列表，含 32 个服务器，分 10 类。你只挑项目需要的。
