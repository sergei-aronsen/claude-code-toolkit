...
|--------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------|
| **Multi-AI валидация планов**        | `/council` — отправляет план в Gemini 3 Pro + GPT 5.2 параллельно для независимого ревью (persona overlays, кеш по хешу, cost gate, ru locale).    |
...
| **Self-learning rules**              | `/learn` сохраняет повторяющиеся фиксы как scoped rule-файлы с `globs:` — авто-загружаются только для релевантных файлов. Без раздувания промпта.  |
| **Постоянная память проекта**        | Факты проекта в `.claude/rules/` — авто-загрузка каждой сессии, в git, переезжают между машинами.                                                  |
...
## Миграция с v3.x или раньше

Если у Вас старая установка toolkit, появившаяся до поддержки Superpowers / GSD, запустите миграционный хелпер — удалит дубликаты с per-file confirmation и полным бэкапом:

```bash
bash <(curl -sSL https://raw.githubusercontent.com/sergei-aronsen/claude-code-toolkit/main/scripts/migrate-to-complement.sh)
```

Полная матрица миграции: [docs/INSTALL.md](../INSTALL.md).

## Рекомендованные MCP-серверы

Каталог интеграций (`/integrations` после установки) покрывает 23 сервера. Минимальный baseline:

| Сервер                  | Назначение                                          |
|-------------------------|-----------------------------------------------------|
| `context7`              | Всегда свежие docs библиотек                        |
| `playwright`            | Browser automation, UI-тесты                        |
| `sequential-thinking`   | Пошаговое решение задач                             |
| `sentry`                | Error monitoring                                    |
| `dbhub` (per-project)   | Универсальный DB-доступ — **только read-only user** |

> **Security note по `dbhub`** — всегда подключайтесь read-only пользователем БД. Не полагайтесь на app-level флаг `--readonly` в DBHub ([известные обходы](https://github.com/bytebase/dbhub/issues/271)). Per-project credentials → `.claude/settings.local.json` (в .gitignore).
