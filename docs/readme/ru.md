...
| **Каталог интеграций**               | TUI-установщик для 31 MCP-серверов + 10 companion CLI в 10 категориях (Backend / Payments / Workspace / Project Management / …). Скоуп per-row.     |
...
Установщик `--integrations` (или `/integrations` после первой установки) показывает TUI-чеклист с 31 серверами в 10 категориях. Берёте только то, что нужно проекту — остальное не трогается.
