...
Установщик `--integrations` (или `/integrations` после первой установки) показывает TUI-чеклист с 23 серверами в 10 категориях. Берёте только то, что нужно проекту — остальное не трогается.
