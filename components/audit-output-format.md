...
Single source of truth for the structured audit report written by `commands/audit.md` (Phase 14). Phase 16 splices this body verbatim into the OUTPUT FORMAT section of every framework prompt under `templates/*/prompts/`. Phase 15's `/council audit-review` parser reads the section headings and slot strings byte-exact — every literal here is a contract. Severity rubric lives in `components/severity-levels.md` and is referenced (never redefined) below.
