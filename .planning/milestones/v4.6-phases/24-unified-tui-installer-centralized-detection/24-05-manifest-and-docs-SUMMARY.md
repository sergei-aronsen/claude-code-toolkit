...
key-decisions:
  ...
  - "Manifest version NOT bumped to 4.6.0 in Phase 24 — deferred to Phase 27 distribution phase per CONTEXT.md Deferred Ideas"
...
**manifest.json wired with 4 new entries (install.sh + lib/{detect2,dispatch,tui}.sh) enabling smart-update auto-discovery; docs/INSTALL.md gains install.sh (unified entry, v4.6+) section with flag table, TUI controls, and BACKCOMPAT-01 note**
...
- `docs/INSTALL.md` gains `## install.sh (unified entry, v4.6+)` section (4 H3 subsections: Quick start, Flags, TUI controls, Backwards compatibility) documenting all 8 flags and TUI key bindings; markdownlint passes
...
- `docs/INSTALL.md` — Added `## install.sh (unified entry, v4.6+)` section with Quick start, Flags table (8 flags), TUI controls table, and Backwards compatibility note

## Decisions Made

- Manifest version left at 4.4.0 — version bump to 4.6.0 is deferred to Phase 27 distribution phase per CONTEXT.md "Deferred Ideas"; `make validate` confirms `version-aligned: 4.4.0`
...
- docs/INSTALL.md contains `## install.sh (unified entry, v4.6+)`: CONFIRMED
