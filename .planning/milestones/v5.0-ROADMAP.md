# v5.0 Milestone Archive — Per-MCP Scope + Project Secrets Boundary

**Status**: ✅ Shipped 2026-05-06
**Phases**: 6 (36–41)
**Plans**: 22/22 complete
**Requirements**: 37/37 closed
**Commits**: 92
**Diff stats**: 79 files changed, 17914 insertions(+), 380 deletions(-)
**Date range**: 2026-05-04 → 2026-05-06 (3 days)

## Goals (delivered)

- Per-row scope toggle in MCP TUI (`[U]/[P]/[L]` glyphs, Tab + `s` hotkeys)
- Project secrets boundary: `<project>/.env` mode 0600 with `${VAR}` substitution in `.mcp.json` (defense-in-depth literal-secret refusal)
- Per-MCP wizard dispatch routing on `TK_MCP_SCOPE` (user/project/local)
- `--mcp-scope=<scope>` CLI flag with broadcast to per-row state in headless paths
- Uninstall secret cleanup: per-MCP + full-toolkit `[y/N]` prompts, `--keep-state` implies `--keep-secrets`, project `.env` never touched
- Calendly catalog entry (official, OAuth-only); Google Workspace explicitly NOT added (claude.ai connectors cover it)
- Catalog validator SCOPE-01 enforcement
- 6 hermetic test scenarios for uninstall secret cleanup; SCOPE-01 + Calendly catalog regressions

## Phase Details (preserved verbatim from ROADMAP.md at close time)
...
See [v5.0-MILESTONE-AUDIT.md](../v5.0-MILESTONE-AUDIT.md) — verdict PASS, all 37 REQ-IDs closed, all baseline tests green.
