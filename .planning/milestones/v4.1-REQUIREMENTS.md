# Requirements Archive: v4.1 Polish & Upstream
...
- [ ] **AUDIT-02**: No template compatibility matrix — no `compatibility.json` to block incompatible stack combos — Status: WONTFIX (KISS — install-time picks 1 framework, no overlay scenario; closed 2026-04-26)
...
- [ ] **AUDIT-04**: No template merge-strategy declaration — base+python+rag overlay semantics undefined — Status: WONTFIX (KISS — no multi-template overlay; framework templates inherit from base only via per-file fallback in installers; closed 2026-04-26)
- [ ] **AUDIT-05**: Relative path assumptions in templates — `../skills/rag.md` inside template markdown breaks post-install — Status: FALSE
- [ ] **AUDIT-06**: No template version pinning — installer pulls main branch, no `template.lock.json` / `template_version` field — Status: WONTFIX (already covered — `manifest.json` ships `version` field, install writes `~/.claude/.toolkit-version`, smart update diffs against it; no lockfile needed for KISS distribution model; closed 2026-04-26)
...
- [x] **AUDIT-10**: No collision detection with existing `.claude/` — overwrite/merge/fail behavior undeclared — Status: Done (already covered — installers use `[ ! -f "$file" ]` idempotent guards, `update-claude.sh` performs SHA256-classified file diff via manifest, `migrate-to-complement.sh` runs three-way diff with `[y/N/d]` prompts; closed 2026-04-26)
- [ ] **AUDIT-11**: No template integrity checksum — no `manifest.hash` — Status: FALSE
- [x] **AUDIT-12**: Markdown commands as templates without linting — required sections/frontmatter/step markers not enforced — Status: Done (closed by HARDEN-A-01 — `scripts/validate-commands.py` + `make validate-commands` enforces `## Purpose` + `## Usage` H2s on all `commands/*.md`; wired into `make check` + CI; closed 2026-04-26)
- [ ] **AUDIT-13**: No dependency graph between templates — rag requires memory, installer doesn't enforce — Status: FALSE
- [x] **AUDIT-14**: No uninstall semantics — can't remove template safely — Status: Done (closed by v4.3 Uninstall — `scripts/uninstall.sh` ships UN-01..UN-08 with SHA256 classify, dry-run, backup, [y/N/d] prompt, idempotency, base-plugin invariant; closed 2026-04-26)
- [x] **AUDIT-15**: No template provenance metadata — no `installed_templates.json` post-install — Status: Done (already covered — v4.0 ships `~/.claude/toolkit-install.json` with mode, detection, per-file SHA256 + path; consumed by `update-claude.sh` and `uninstall.sh`; closed 2026-04-26)
...
| AUDIT-02 | Phase 12 | Closed - WONTFIX (KISS, 2026-04-26) |
| AUDIT-03 | Phase 12 | Closed - FALSE |
| AUDIT-04 | Phase 12 | Closed - WONTFIX (KISS, 2026-04-26) |
| AUDIT-05 | Phase 12 | Closed - FALSE |
| AUDIT-06 | Phase 12 | Closed - WONTFIX (already covered by manifest.json + .toolkit-version, 2026-04-26) |
| AUDIT-07 | Phase 12 | Closed - FALSE |
| AUDIT-08 | Phase 12 | Closed - FALSE |
| AUDIT-09 | Phase 12 | Closed - FALSE |
| AUDIT-10 | Phase 12 | Done (already covered by idempotent install + SHA256 diff, 2026-04-26) |
| AUDIT-11 | Phase 12 | Closed - FALSE |
| AUDIT-12 | Phase 12 | Done (closed by HARDEN-A-01) |
| AUDIT-13 | Phase 12 | Closed - FALSE |
| AUDIT-14 | Phase 12 | Done (closed by v4.3 Uninstall, 2026-04-26) |
| AUDIT-15 | Phase 12 | Done (already covered by toolkit-install.json, 2026-04-26) |
