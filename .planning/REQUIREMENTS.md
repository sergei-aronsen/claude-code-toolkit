...
- [x] **BACKUP-01**: `scripts/update-claude.sh --clean-backups` scans `~/.claude-backup-<epoch>-<pid>` and `~/.claude-backup-pre-migrate-<epoch>` dirs (real on-disk patterns), prompts per dir with size + age, removes on `[y/N]` confirmation. Supports `--keep N` to preserve the N most recent.
- [x] **BACKUP-02**: Any script that creates a new backup dir also checks total count under `~/.claude-backup-*/` and `~/.claude-backup-pre-migrate-*/`. If count exceeds 10, print a one-line warning pointing to `update-claude.sh --clean-backups`. Non-fatal.
