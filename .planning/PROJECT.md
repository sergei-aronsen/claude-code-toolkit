...
- **v5.0 Per-MCP Scope + Project Secrets Boundary** (2026-05-06) — 6 phases (36–41), 16 plans, 37 REQ-IDs (SCOPE-01..03, TUI-SCOPE-01..05, SEC-01..06, DISP-01..04, UN-SEC-01..05, INT-13/14, TEST-01..06, DIST-01..03, DOCS-01..03). Per-row scope toggle in MCP TUI (`[U]/[P]/[L]` glyphs, Tab cycles single row, `s` set-all 3-state). New `scripts/lib/project-secrets.sh` (4 functions): `_write_env` (mode 0600 atomic mktemp+mv+chmod), `_ensure_gitignore` (idempotent `.env` line append), `_render_mcp_env_block` (`${VAR}` substitution JSON), `_validate_mcp_env_block` (defense-in-depth literal-secret refusal). `mcp_wizard_run` learns scope routing: project → `<project>/.env` + `claude mcp add --scope project -e KEY=${KEY}`; user/local → `~/.claude/mcp-config.env` (byte-identical to v4.6/v4.9). 4-tuple deferred-secrets queue. `--mcp-scope=user|local|project` CLI flag with broadcast to per-row state in headless paths (`--yes`, `TK_MCP_PRE_SELECTED`). Uninstall secret cleanup: per-MCP `[y/N]` `mcp-config.env` cleanup prompt + full-toolkit prompt + project `.env` files NEVER touched + `--keep-state` implies `--keep-secrets`. Calendly catalog entry (official, OAuth-only); Google Workspace explicitly NOT added (claude.ai connectors cover it). Catalog validator SCOPE-01 enforcement. Manifest 5.0.0 + 3 plugin.json (`tk-skills`/`tk-commands`/`tk-framework-rules`) bumped. CHANGELOG `[5.0.0]` consolidated. `docs/INTEGRATIONS.md` Per-MCP Scope section + INSTALL.md `--mcp-scope` row + new `## Uninstall` section + README Killer Features row. 6 hermetic UN-SEC scenarios + SCOPE-01 + Calendly catalog regressions. All baselines green: test-mcp-selector 36, test-mcp-wizard 53, test-mcp-secrets 11, test-project-secrets 42, test-uninstall-state-cleanup 17, test-uninstall-prompt 10, test-uninstall-keep-state 11, test-integrations-catalog 20. Ready to tag `v5.0.0`.
- **v4.9 Integrations Catalog** (2026-05-02) — 4 phases (32–35), 14 plans, 36 REQ-IDs (CAT-01..04, CLI-01..04, TUI-01..05, INT-01..12, DROP-01, EXIST-01, DOCS-01..05, TEST-01..04, DIST-01..02). Schema migration `mcp-catalog.json` → `integrations-catalog.json` (`schema_version: 2`) with `categories[]` + `components.mcp{}` + `components.cli{}`. `scripts/lib/cli-installer.sh` ships `cli_detect` / `cli_install` / `cli_post_install_hint` primitives (cross-platform via `uname -s`, brew-absent fallback, NO sudo auto-prefix). 12 new integrations across 10 categories: supabase + cloudflare + aws-{cost-explorer,cloudwatch-logs} (Backend) + stripe (Payments) + youtrack/linear/jira (Project Management) + figma (Design) + slack + telegram (Communication) + notebooklm (Research). `unofficial: true` confirm gate on telegram + notebooklm. Phase 34 TUI redesign: category-grouped rendering, per-row MCP/CLI status glyphs, `--mcp-only` / `--cli-only` mutex flags, closing per-component summary table. Phase 35 close: manifest 4.8.0 → 4.9.0, `init-claude.sh --version` parity, 3 new hermetic test suites (`test-integrations-catalog` PASS=14 floor 10, `test-cli-installer` PASS=24 floor 8, `test-integrations-tui` PASS=36 floor 15), `docs/INTEGRATIONS.md` (208 lines, complete reference + Global-vs-per-project boundary), INSTALL.md 4 new flag rows, README Killer Features bullet, CHANGELOG `[4.9.0]` consolidated. Validator `scripts/validate-integrations-catalog.py` (Python stdlib only) + Makefile/CI `make validate-catalog`. `--mcps` flag deprecated as alias for `--integrations`. `sequential-thinking` removed (DROP-01). All 4 baselines preserved (test-mcp-selector 21, test-bootstrap 26, test-install-tui 52, test-integrations-foundation 32). Ready to tag `v4.9.0`.
- **v4.8 Multi-CLI Bridge** (2026-04-29) — 4 phases (28–31), 12 plans, 35 tasks, 18 REQ-IDs (BRIDGE-DET-01..03, BRIDGE-GEN-01..04, BRIDGE-SYNC-01..03, BRIDGE-UN-01..02, BRIDGE-UX-01..04, BRIDGE-DIST-01..02, BRIDGE-TEST-01, BRIDGE-DOCS-01..02). `scripts/lib/bridges.sh` (700+ lines) ships `bridge_create_project` / `bridge_create_global` writing `GEMINI.md` (Gemini CLI) + `AGENTS.md` (OpenAI Codex CLI — NOT `CODEX.md`, OpenAI standard) with byte-identical 4-line auto-generated HTML banner; registers in `~/.claude/toolkit-install.json` `bridges[]` with both `source_sha256` and `bridge_sha256`. `update-claude.sh` `sync_bridges()` decision tree: SKIP / ORPHANED / drift `[y/N/d]` / REWRITE / in-sync. `--break-bridge <target>` / `--restore-bridge <target>` flags flip `user_owned`. `install.sh` TUI conditional rows for detected CLIs + dispatch shim. `init-claude.sh` / `init-local.sh` post-install per-CLI prompt with `TK_BRIDGE_TTY_SRC` test seam, fail-closed N on no-TTY. `--no-bridges` + `TK_NO_BRIDGES=1` opt-out, `--bridges gemini,codex` force-create with `--fail-fast` exit 1 on absent CLI. `state.sh::write_state` extended to 10-arg with `bridges_json` passthrough (backward-compatible default `'[]'`). `uninstall.sh` includes bridges in REMOVE_LIST + `[y/N/d]` for modified bridges + state purge gated on `--keep-state`. 3 new hermetic test suites: `test-bridges-foundation.sh` (PASS=5), `test-bridges-sync.sh` (PASS=25), `test-bridges-install-ux.sh` (PASS=20) + `test-bridges.sh` aggregator (PASS=50). 3 WR bugs fixed inline. Manifest 4.7.0 → 4.8.0. `docs/BRIDGES.md` + INSTALL.md flag rows + README Killer Features. BACKCOMPAT-01 preserved. Tagged `v4.8.0`.
- **v4.7 Council Rework SP2-11** (2026-04-29) — Phase 24 sub-phases 2 through 11. 28 atomic commits on `feature/v4.8-council-rework-sp2-11` (PR #13). `make check` zero failures, audit-review test 81/81. Manifest 4.6.0 → 4.7.0. Highlights: editable system prompts with `.upstream-new` sidecar (SP2), context enrichment + redaction (SP3), `usage.jsonl` + `pricing.json` + `/council-stats` + cost-confirm gate (SP4), Codex CLI provider + max reasoning + OpenRouter fallback chain (SP5), content-hash cache + `/council clear-cache` + `--no-cache` (SP6), `council-integration` skill (SP7), domain personas + `--dry-run` + `--format json` + TL;DR + `--mode retro` (SP8), Russian translations + `--lang auto` Cyrillic detect (SP9), `commands/council.md` rewrite + new `docs/COUNCIL.md` (SP10), `mcp-server.py` stdio JSON-RPC for Claude Desktop (SP11).
...
_None defined — v5.0 just shipped 2026-05-06. Run `/gsd-new-milestone` to scope the next milestone (questioning → research → requirements → roadmap)._

**Carry-overs available for next milestone scoping** (from v5.0 close):
...
- `--keep-secrets` independent flag (deferred to v5.1+ if friction surfaces; YAGNI for v5.0).
- macOS Keychain / Linux libsecret integration (SEC-FUT-01).
- 1Password / Vault detection (SEC-FUT-02).
- Per-MCP scope env-var override `TK_MCP_SCOPE_<name>=user` (SCOPE-FUT-01).
...
Phase 24 (`.planning/phases/24-council-globalize/PLAN.md`) is now closed: SP1 in v4.5, SP2-11 in v4.7 (PR #13).
...
---
*Last updated: 2026-05-06 — **v5.0 Per-MCP Scope + Project Secrets Boundary** SHIPPED via `/gsd-complete-milestone v5.0` — 6 phases (36–41), 16 plans, 37 REQ-IDs. Per-row scope toggle in TUI (`[U]/[P]/[L]`), `scripts/lib/project-secrets.sh` library, wizard scope routing (project → `<project>/.env` + `${VAR}` substitution, defense-in-depth literal-secret refusal), uninstall secret cleanup prompts, Calendly catalog entry (Google Workspace deliberately not added). Previous: **v4.9 Integrations Catalog** shipped 2026-05-02 — 4 phases (32–35), 14 plans, 36 REQ-IDs. **v4.8 Multi-CLI Bridge** shipped 2026-04-29 — 4 phases (28–31), 18/18 BRIDGE-* REQ-IDs. **v4.7 Council Rework SP2-11** shipped on `feature/v4.8-council-rework-sp2-11` (PR #13). **v4.6 Install Flow UX & Desktop Reach** shipped via `/gsd-complete-milestone v4.6` — 4 phases (24–27), 36/36 REQ-IDs.*
