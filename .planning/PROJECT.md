...
- **v4.7 Council Rework SP2-11** (2026-04-29) — Phase 24 sub-phases 2 through 11. 28 atomic commits on `feature/v4.8-council-rework-sp2-11` (PR #13). `make check` zero failures, audit-review test 81/81. Manifest 4.6.0 → 4.7.0. Highlights: editable system prompts with `.upstream-new` sidecar (SP2), context enrichment + redaction (SP3), `usage.jsonl` + `pricing.json` + `/council-stats` + cost-confirm gate (SP4), Codex CLI provider + max reasoning + OpenRouter fallback chain (SP5), content-hash cache + `/council clear-cache` + `--no-cache` (SP6), `council-integration` skill (SP7), domain personas + `--dry-run` + `--format json` + TL;DR + `--mode retro` (SP8), Russian translations + `--lang auto` Cyrillic detect (SP9), `commands/council.md` rewrite + new `docs/COUNCIL.md` (SP10), `mcp-server.py` stdio JSON-RPC for Claude Desktop (SP11). PLAN deviations: SP7 GSD commands ship upstream so integration is a skill rather than command patches; SP10 version bumped to next clean minor instead of spec'd v4.5.0 (already taken).
...
v4.7 ships Phase 24 SP2-11 (Council rework) on `feature/v4.8-council-rework-sp2-11` via PR #13.
...
v4.8 scope to be defined via `/gsd-new-milestone` when ready. Phase 24 (`.planning/phases/24-council-globalize/PLAN.md`) is now closed: SP1 in v4.5, SP2-11 in v4.7 (PR #13).
...
---
*Last updated: 2026-04-29 — **v4.7 Council Rework SP2-11** shipped on `feature/v4.8-council-rework-sp2-11` (PR #13). Phase 24 sub-phases 2 through 11 closed: 28 atomic commits, manifest 4.6.0 → 4.7.0, audit-review test 81/81, `make check` zero failures. Awaiting PR review + merge to tag `v4.7.0`. Previous: **v4.6 Install Flow UX & Desktop Reach** shipped via `/gsd-complete-milestone v4.6` — 4 phases (24–27), 17 plans, 42 tasks, 36/36 REQ-IDs.*
