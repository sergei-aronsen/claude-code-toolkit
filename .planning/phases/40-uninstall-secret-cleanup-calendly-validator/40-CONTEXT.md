...
`scripts/validate-integrations-catalog.py` (294 lines pre-existing) gains an assertion: every MCP entry MUST carry `default_scope` field with value in `{"user", "local", "project"}`. Same enum that `mcp_catalog_load` enforces (Phase 36 SCOPE-01). Assertion runs alongside existing schema checks; failure exits non-zero with line-pointer to the offending entry. No new file. The TEST-06 row in REQUIREMENTS.md is already marked `[x]` — verify the implementation actually exists; if it does (probable, since Phase 36 likely landed it), Phase 40 adds the regression test in `test-integrations-catalog.sh` instead of re-implementing. Planning resolves this ambiguity by reading both files first.
