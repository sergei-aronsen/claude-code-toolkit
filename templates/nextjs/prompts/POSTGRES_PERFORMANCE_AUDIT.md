...
- Analysis via `pg_stat_statements` (real statistics)
...
**avg_rows_per_seq_scan > 1000** + frequent seq_scan — needs an index.
...
### 11.2 Prisma-Specific

```typescript
// Prisma migrations use raw SQL under the hood
// For large tables, edit the generated SQL migration before applying:

// ❌ Generated migration (dangerous on large tables)
// ALTER TABLE "Site" ADD COLUMN "status" TEXT NOT NULL;

// ✅ Manually edit migration.sql
// ALTER TABLE "Site" ADD COLUMN "status" TEXT;
// -- Backfill in application code, then add constraint
```

### 11.3 Checklist

- [ ] `NOT NULL` columns added with `DEFAULT` value (instant in PG 11+)
- [ ] Indexes created with `CONCURRENTLY` (edit Prisma migration SQL manually)
