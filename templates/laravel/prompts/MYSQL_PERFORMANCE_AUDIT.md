...
- Finding queries with bad scan ratio
...
For full audit you need access to `performance_schema`. If not - use `debian-sys-maint` (password in `/etc/mysql/debian.cnf`).
...
- Parallel workers update the same record
...
### 9.2 Laravel-Specific

```php
// ❌ Dangerous on large tables
Schema::table('sites', function (Blueprint $table) {
    $table->string('status'); // May lock table
});

// ✅ For very large tables — use pt-online-schema-change
// pt-online-schema-change --alter "ADD COLUMN status VARCHAR(50) NOT NULL DEFAULT 'active'" D=mydb,t=sites
```
