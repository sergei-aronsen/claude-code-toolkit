...
- Guessing "what indexes are needed" without data
...
| ------- | ------ | ------ |
| < 60% | OK | - |
| 60-80% | Warning | Plan to increase or use connection pooling |
...
### Queries with poor plan (many rows scanned)
...
### 11.2 Go-Specific (golang-migrate / goose)

```go
// In migration files, use raw SQL for safety
// ❌ Dangerous
const migrationUp = `ALTER TABLE sites ADD COLUMN status TEXT NOT NULL;`

// ✅ Safe
const migrationUp = `
ALTER TABLE sites ADD COLUMN status TEXT;
-- Backfill in application code, then:
