...
> Universal guide for MySQL/MariaDB performance audit
...
- Analysis via `performance_schema` (real statistics)
...
Full audit requires access to `performance_schema`. If not available - use `debian-sys-maint` (password in `/etc/mysql/debian.cnf`).
...
| ------ | ------ | ------ |
| < 60% | OK | - |
| 60-80% | Warning | Plan to increase |
